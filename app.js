...
async function loadCategoryRows() {
  ...
  if (activeSection === 'trending') {
    ...
    const allTrend = await Promise.all(
      mts.flatMap(mt => [1,2,3,4,5].map(p => directFetch(`/trending/${mt}/day`, { page: p })))
    );
...
async function fetchCategoryRow(cat, track, globalPoolIds = null, usedIds = new Set()) {
  ...
  if (cat.type === 'collection') {
    ...
    // Build base params for this category
    const buildCatParams = (extra = {}) => {
      const p = { sort_by: 'popularity.desc', without_genres: '10749', ...extra };
      if (cat.type === 'company') p.with_companies = cat.value;
      if (cat.type === 'keyword') p.with_keywords  = cat.value;
      if (cat.type === 'genre')   p.with_genres    = cat.value;
      if (cat.type === 'multi')   { p.with_genres = '16'; p.with_keywords = '210024'; }
      return p;
    };

    // Curated types (company, keyword, multi) always fetch directly — never intersect with
    // trending/popular pool since niche categories often have 0 overlap with global top pages.
    const isCurated = cat.type === 'company' || cat.type === 'keyword' || cat.type === 'multi';

    if (activeSection === 'trending' && !isCurated) {
      // Genre rows: try intersecting with trending pool, but always guarantee enough items
      let trendIds = globalPoolIds;
      if (!trendIds) {
        const trendPages = await Promise.all([1,2,3,4,5].map(p => directFetch(`/trending/${mt}/day`, { page: p })));
        trendIds = new Set(trendPages.filter(d => d?.results).flatMap(d => d.results).map(i => i.id));
      }
      const catPages = await Promise.all([1,2,3,4,5].map(p => directFetch(endpoint, { ...buildCatParams(), page: p })));
      const catItems = catPages.filter(d => d?.results).flatMap(d => d.results);
      const trendIntersect = catItems.filter(i => trendIds.has(i.id) && !usedIds.has(i.id));
      if (trendIntersect.length >= 10) {
        allItems = trendIntersect;
      } else {
        // Not enough trending matches — fall back to newest releases for this genre (feels timely)
        const fallbackPages = await Promise.all([1,2,3].map(p =>
          directFetch(endpoint, { ...buildCatParams({ sort_by: mt === 'tv' ? 'first_air_date.desc' : 'primary_release_date.desc', 'vote_count.gte': 5 }), page: p })
        ));
        const fallbackItems = fallbackPages.filter(d => d?.results).flatMap(d => d.results).filter(i => !usedIds.has(i.id));
        allItems = fallbackItems.length >= 10 ? fallbackItems : [...trendIntersect, ...catItems.filter(i => !usedIds.has(i.id))];
      }

    } else if (activeSection === 'toprated') {
      const pages = await Promise.all([1,2,3].map(p =>
        directFetch(endpoint, { ...buildCatParams({ sort_by: 'vote_average.desc', 'vote_count.gte': 500 }), page: p })
      ));
      allItems = pages.filter(d => d?.results).flatMap(d => d.results);

    } else if (activeSection === 'popular' && !isCurated) {
      // Genre rows: intersect with popular pool
      ...
      const catPages = await Promise.all([1,2,3,4,5].map(p => directFetch(endpoint, { ...buildCatParams(), page: p })));
      const catItems = catPages.filter(d => d?.results).flatMap(d => d.results);
      const popIntersect = catItems.filter(i => popularIds.has(i.id) && !usedIds.has(i.id));
      if (popIntersect.length >= 10) {
        allItems = popIntersect;
      } else {
        // Not enough popular matches — fall back to most popular for this genre
        allItems = catItems.filter(i => !usedIds.has(i.id));
        if (allItems.length < 10) allItems = catItems;
      }

    } else if (activeSection === 'trending' && isCurated) {
      // Curated categories (Marvel, DC, etc.) in Trending: newest releases first
      const trendDateSort = mt === 'tv' ? 'first_air_date.desc' : 'primary_release_date.desc';
      const pages = await Promise.all([1,2,3].map(p =>
        directFetch(endpoint, { ...buildCatParams({ sort_by: trendDateSort, 'vote_count.gte': 5 }), page: p })
      ));
      allItems = pages.filter(d => d?.results).flatMap(d => d.results);

    } else if (activeSection === 'popular' && isCurated) {
      // Curated categories (Marvel, DC, etc.) in Popular: all-time most popular
      const pages = await Promise.all([1,2,3].map(p =>
        directFetch(endpoint, { ...buildCatParams({ sort_by: 'popularity.desc' }), page: p })
      ));
      allItems = pages.filter(d => d?.results).flatMap(d => d.results);

    } else {
      // Default home or other section
      const pages = await Promise.all([1,2,3].map(p => directFetch(endpoint, { ...buildCatParams(), page: p })));
      allItems = pages.filter(d => d?.results).flatMap(d => d.results);
      if (!isCurated && activeSection === 'home') allItems = shuffle(allItems);
    }
  }
  ...
  if (cat.type === 'collection') {
    ...
    // Trending: newest releases first for all category types
    sorted = allItems.sort((a, b) => (b.release_date || b.first_air_date || '').localeCompare(a.release_date || a.first_air_date || ''));
  } else {
    // Popular and others: highest popularity first
...
async function fetchHeroItems(tab, mediaType) {
  ...
  let endpoint;
  if (tab === 'trending') endpoint = mt === 'tv' ? '/trending/tv/day'  : '/trending/movie/day';
  ...
  if (mediaType === null) {
    const epMovie = tab === 'trending' ? '/trending/movie/day' : tab === 'popular' ? '/movie/popular' : '/movie/top_rated';
    const epTv    = tab === 'trending' ? '/trending/tv/day'    : tab === 'popular' ? '/tv/popular'    : '/tv/top_rated';
...
async function loadSection(section, page = 1) {
  ...
  if (mt === 'movie') {
    switch (section) {
      case 'trending': endpoint = '/trending/movie/day'; title = 'Trending Movies';    break;
    ...
    switch (section) {
      case 'trending': endpoint = '/trending/tv/day'; title = 'Trending Shows';         break;
    ...
    if (preserveSection) {
      ...
      if (preserveSection === 'favorites') {
        ...
        loadGenres().then(() => {
          ...
          else if (preserveSection === 'upcoming') { _upcomingFilter = mt; loadUpcomingDefault(); }
